...
`DatecodeNumAdapter.run()` SHALL 接受一張圖片（`image: np.ndarray`）及可選參數 `YMD_option`（1=日/月/年，2=月/日/年，預設 1）、`include_diag`（bool）、`debug`（bool），並呼叫 `datecode_num_st` 的 `proc()` 回傳辨識結果。
