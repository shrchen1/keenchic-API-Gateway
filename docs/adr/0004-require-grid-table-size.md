...
Valid dimensions use a row count from 1 through 8 or exactly 15 and a column count from 1 through 8. The `[1, 1]` size is valid for Grid Inspection and returns a one-row, one-column `pred_text_L` matrix; Cell Inspection remains available when a caller needs its existing single-cell contract. Unsupported dimensions return HTTP 422 before inference. The column limit also prevents excessive `"N/G"` padding from consuming unbounded CPU and memory.
